...
export const renderTextWithLatex = html => {
  ...
  const hasLatex =
    html.includes("\\(") || html.includes("\\)") || html.includes("\\[") || html.includes("\\]");

  if (hasLatex) {
    // For content with LaTeX, we need to preserve the HTML structure
    // but ensure MathJax can process the LaTeX expressions
    return (
      <MathJax dynamic>
    ...
    // For plain text, strip HTML tags and render as text
    const tmp = document.createElement("div");
    tmp.innerHTML = html;
    const safeText = tmp.textContent || tmp.innerText || "";

    return <MathJax dynamic>{safeText}</MathJax>;
  }
};
              ...
              <SolutionStep>
                ...
                >
                  <Typography variant="body1" sx={{ color: "#333", lineHeight: 1.6, flex: 1 }}>
                    {renderTextWithLatex(question.solution)}
                  </Typography>
                </Box>
              </SolutionStep>
            ) : hasStepsSolution ? (
        ...
        <QuestionsContainer>
          <QuestionCard variant="outlined">
            ...
            >
                    ...
                    <Box sx={{ flex: 1, mr: 2 }}>
                      ...
                      >
                        ...
                        {question.negativeMarking && (
                          <Chip
                            label={`-${question.negativeMarksValue || 0}`}
                            size="small"
                            color="error"
                            variant="outlined"
                          />
              ...
              <SolutionSection>{renderSolutionContent(question, solution)}</SolutionSection>
            </CardContent>
          </QuestionCard>
        </QuestionsContainer>
      </Box>
