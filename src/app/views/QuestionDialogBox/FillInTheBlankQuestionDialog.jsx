...
export const FillInTheBlankQuestionDialog = ({ open, onClose, onCreateQuestion, editQuestion, onSaveSubQuestion, comprehensive }) => {
  const [formData, setFormData] = useState({
    ...
    negativeMarksValue: '',
    blanks: [{ id: `blank-${Date.now()}-0`, correctAnswer: '' }],
  ...
  const [showBlankLatexEditor, setShowBlankLatexEditor] = useState({});
  const [questionLatex, setQuestionLatex] = useState('');
  const [solutionLatex, setSolutionLatex] = useState('');
  const [blankLatex, setBlankLatex] = useState({});
  ...
  useEffect(() => {
    if (editQuestion) {
      setFormData({
        ...
        blanks: editQuestion.blanks ? editQuestion.blanks.map(blank => ({
          ...blank,
  ...
  const handleBlankAnswerChange = (index) => (event) => {
    const newBlanks = [...formData.blanks];
    newBlanks[index].correctAnswer = event.target.value;
    setFormData(prev => ({
      ...prev,
      blanks: newBlanks
    }));
  ...
  const handleAddBlank = () => {
    setFormData(prev => ({
      ...prev,
      blanks: [...prev.blanks, { id: `blank-${Date.now()}-${formData.blanks.length}-${Math.random()}`, correctAnswer: '' }]
    }));
  };

  const handleRemoveBlank = (indexToRemove) => () => {
    const newBlanks = formData.blanks.filter((_, i) => i !== indexToRemove);
    setFormData(prev => ({
      ...prev,
      blanks: newBlanks
    }));
  ...
  const handleBlankLatexChange = (index) => (latex) => {
    setBlankLatex(prev => ({
      ...prev,
      [index]: latex
    }));
  ...
  const insertLatexIntoBlank = (index) => () => {
    const latex = blankLatex[index];
    if (latex && latex.trim()) {
      const newBlanks = [...formData.blanks];
      newBlanks[index].correctAnswer = `${newBlanks[index].correctAnswer} $${latex}$`;
      setFormData(prev => ({
        ...prev,
        blanks: newBlanks
      }));
  ...
  const toggleBlankLatexEditor = (index) => () => {
    setShowBlankLatexEditor(prev => ({
      ...prev,
      [index]: !prev[index]
    }));
  };
  ...
  const handleSaveQuestion = async () => {
    ...
    if (formData.blanks.length === 0 || formData.blanks.some(blank => !blank.correctAnswer.trim())) {
      Swal.fire({
        icon: 'warning',
        title: 'Validation Error',
        text: 'At least one blank is required, and all blank answers must be filled.',
        confirmButtonColor: '#f39c12'
      });
      return;
    }
    ...
    try {
      ...
      console.error(`Error ${editQuestion ? 'updating' : 'creating'} Fill in the Blank question:`, error);
      Swal.fire({
        icon: 'error',
        title: 'Error',
    ...
    >
      <StyledDialogTitle>
        <Typography variant="h6" component="div">
          {editQuestion ? 'Edit Fill in the Blank Question' : 'Create Fill in the Blank Question'}
        </Typography>
        <IconButton
          aria-label="close"
          onClick={onClose}
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box>
            ...
            {showQuestionPreview ? (
              ...
              >
                {renderTextWithLatex(formData.questionText.replace(/\[BLANK\]/g, '______'))}
              </Box>
            ) : (
              <TextField
                autoFocus
                margin="dense"
                label="Question Text (Use [BLANK] for blank spaces)"
                type="text"
                fullWidth
                multiline
                rows={4}
                ...
                helperText={
                  formData.questionText !== '' && !formData.questionText.trim()
                    ...
                    : 'Example: The capital of France is [BLANK]. Use $...$ for inline math, $...$ for display math'
                }
              />
            )}
          </Box>
          ...
          {/* Blanks and Correct Answers with LaTeX Support */}
          ...
          {formData.blanks.map((blank, index) => (
            <Box key={blank.id} sx={{ mb: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                ...
                <Tooltip title="Toggle LaTeX Editor">
                  <Button
                    size="small"
                    variant="outlined"
                    onClick={toggleBlankLatexEditor(index)}
                    startIcon={<EditIcon />}
                    color={showBlankLatexEditor[index] ? 'primary' : 'inherit'}
                  >
                    LaTeX
                  </Button>
                </Tooltip>
                {formData.blanks.length > 1 && (
                  <IconButton onClick={handleRemoveBlank(index)} color="error" size="small">
                    <DeleteIcon />
                  </IconButton>
                )}
              </Box>

              {showBlankLatexEditor[index] && (
                <Box sx={{ mb: 2 }}>
                  <LaTeXEditor
                    value={blankLatex[index] || ''}
                    onChange={handleBlankLatexChange(index)}
                    placeholder={`Enter mathematical expressions for blank ${index + 1}...`}
                    compact={true}
                  />
                  <Box sx={{ mt: 1, display: 'flex', gap: 1 }}>
                    <Button
                      size="small"
                      variant="outlined"
                      onClick={insertLatexIntoBlank(index)}
                      disabled={!blankLatex[index] || !blankLatex[index].trim()}
                    >
                      Insert into Answer
                    </Button>
                    <Button
                      size="small"
                      variant="text"
                      onClick={() => setBlankLatex(prev => ({ ...prev, [index]: '' }))}
                    >
                      Clear
                    </Button>
                  </Box>
              ...
              <TextField
                ...
                onChange={handleBlankAnswerChange(index)}
                required
                error={blank.correctAnswer !== '' && !blank.correctAnswer.trim()}
                helperText={
                  blank.correctAnswer !== '' && !blank.correctAnswer.trim()
